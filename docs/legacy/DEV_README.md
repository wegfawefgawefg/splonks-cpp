...
- There are functions for hurting an ent, healing an ent etc. These have to be used rather than just setting an ents hp.
...
- Never make game logic depend on EntRenderState.
- deadness is used on undieable ents to signal that they should blow up in some way and delete themself (like rock or jetpack, or bomb).
- block crushing doesnt work on things grounded under 5, 5 in size. (This may no longer be true.)

- ent_just_collided is a semi dangerous function that can be really annoying to the player if overused
  there shouldnt be any issues using it on ents that are supposed to explode on contact with something.
- ents should not set themselves to super state dead, just hurt the ent, and let the death checker handle it.
...
- Try to keep systems generic. This wont always be avoidable, but the systems are itterating through all the ents,
  so if you can make a generic system you reduce the full num_system \* num_ents steps.
