...
Status: historical host-authoritative locomotion/prediction plan. The
active experiment is now `docs/plans/input_lockstep_rollback_plan.md`.

Under input lockstep/rollback, most of these host plausibility checks
...
policy belongs in the rollback/input layer rather than in ent movement code.

Purpose: define where the owning player is allowed to be more authoritative than the host's exact delayed simulation. This is not durable world authority. It is bounded local locomotion authority for responsiveness.

The core problem: edge-sensitive movement states such as hanging, climbing, coyote jumps, and swimming can be correct on the owning client but missed by the host because the host sees input and position a few frames late. If the host blindly rejects those transitions, the player snaps out of valid local movement and the game feels broken.

## Ownership Boundary

- The host owns durable/shared outcomes: tile changes, ent spawns, damage, pickup/economy, shop aggro, exits, stage progression, and canonical snapshots.
- A free player is locally body-authoritative: the owning client may claim responsive body state for its own player.
- The host validates claimed locomotion/body state with generous plausibility checks, then broadcasts the accepted body state.
- Peers render accepted host state for remote players and do not independently decide remote hang/climb/jump states.
- Invalid claims are corrected by explicit host repair. A delayed ordinary host position echo is not by itself a repair command for the owning peer, because high-latency echoes otherwise fight local prediction and create host-position/local-position warping.
- Valid claims should be accepted even if exact host replay missed the same ledge or coyote window.

This follows the same broad multiplayer goal as our Terraria/tModLoader-style networking: gameplay facts converge through host-owned lanes, but local movement should not wait for a round trip or exact delayed replay.

## Authority Modes

Use these modes conceptually. They do not need to be a copied enum on every ent unless the code needs one later.

- `FreeLocalOwned`: ordinary walking, running, jumping, falling, hanging, climbing, and swimming. The owning client sends body claims; the host validates and accepts/rejects.
- `HostExternal`: carried, held, attached, thrown by another ent, stunned, dead, crushed, teleported, stage-transition locked, or under scripted motion. The host owns body state; owner input is still accepted as requests, but not as position authority.
- `HostInterrupt`: a short external-control window after damage, explosion impulse, proj contact, forced drop, or throw. The host owns initial body impulse/state; the owner regains free body authority once the interrupt resolves.

Rule: `FreeLocalOwned` is the only mode that may apply ordinary player body claims. A held/stunned/dead/thrown player must not be able to overwrite host body state through the high-frequency player snapshot lane.
...
- [ ] Stage instance and player id match current host state.
- [ ] The claim sequence/frame is newer than the last accepted locomotion claim.
- [ ] Claimed position is within a bounded distance from the host's last accepted/predicted body position.
...
- [x] Reject if dead, hard-stunned, deactivated, or host-carried unless the transition explicitly allows it.
- [x] Reject if the host has the player held, attached, thrown, stunned, dead, or under stage transition.
...
- [ ] The host currently sees the player as `FreeLocalOwned`.
- [ ] Claimed AABB does not overlap solid host tiles.
- [ ] Claimed position is near the current host body within latency tolerance.
- [ ] Claimed velocity is within generous player movement/impulse bounds.
- [ ] If claimed grounded, the host sees valid floor support or stage-bottom support at the claimed position.
...
Reason: this is the Terraria-like fix for one-block ledges and micro-tap jumps. The host should not re-decide whether a remote player barely made the platform from delayed inputs; it should accept a plausible final body state.
...
Reason: a client can correctly hit a one-frame ledge window that the host misses. Exact replay is too strict for hanging.
...
The host should not accept repeated coyote jumps without a landing/reset.
...
- [ ] Host sees fluid at or near that area, with a small tile tolerance because fluids are dynamic.
...
- [ ] Holder relationship is host-approved.
- [ ] Held player can request jump-out/drop through a legal action.
- [ ] Throw velocity comes from a host-approved throw action.
...
This prevents the host from mowing over horizontal throw velocity while still allowing a thrown player to steer once free.

## Fall Damage

Fall damage is a shared gameplay outcome and should stay host-owned.

Rules:

- [ ] Host computes health/stun damage.
- [ ] Client may predict local fall damage pres, but host result wins.
- [ ] Host fall timer must use accepted client locomotion states such as hang, climb, and fluid contact so stale remote replay does not create bogus fall damage.
- [ ] If accepted hang/climb/swim states reset or pause fall timer, the host's body history must reflect that before damage calculation.
...
contact outcomes, while the host still commits the durable result.
...
- A peer may claim hits caused by a locally controlled interaction source: the
  ...
  fired proj, or a tool artifact.
- A peer should not freely claim contested player-vs-player harm. The
  host should validate those with both players' latest accepted body
  snapshots.
- The host owns durable outcomes: health, death, loot spawns, tile
  destruction, shop anger, money, ownership, and stage progression.
- The peer may immediately predict local pres/body feel: damage flashes,
  stun pose, knockback, hit particles, and similar temporary visual response.
  Host messages later repair or confirm the durable state.

Plausibility checks for host-accepted contact claims:

- [ ] Source ent is owned by, held by, or causally attributed to the sending
  player.
...
- [ ] Source and target AABBs overlap, swept-overlapped recently, or are within a
  latency-aware tolerance of the claimed attack/proj shape.
- [ ] Weapon/tool/use state is compatible with the claim: attack frame active,
  proj contact timer active, thrown source still dangerous, or contact
...
the existing typed `DamageEntAction` / `HitEntAction` request lane, add
shared validation and prediction policy there, and let ordinary ent patches
...
- [ ] Add host validators for ordinary free body, hang, glove/corner hang, climb attach, jump/coyote jump, swim impulse, and carried jump-out/release.
- [x] Apply valid hang/climb/jump claims to host body state before fall-damage/contact resolution.
- [x] Apply valid ordinary free body claims to host body state before fall-damage/contact resolution.
- [x] Broadcast accepted hang/climb/jump state through existing player body snapshots.
- [x] Do not hard-snap the owning peer to ordinary delayed host free-movement snapshots.
- [ ] Reject invalid claims with explicit body repair, not ordinary delayed position echo and not special-case item logic.
- [ ] Add debug overlay for local predicted state versus host accepted state.
...
- [ ] Add peer-side predicted contact pres/body response for local-player
  damage and local-source hits.
- [ ] Add host-side plausibility validation for peer
  `DamageEntAction` / `HitEntAction` claims.
- [ ] Add packet/frame tests for peer local-source bat/knife/proj hit,
...
owning peer as movement repairs. `UseHeldEnt` / `UseBackEnt` patches the
...
Items that directly modify the holder's body opt into local attach-use
prediction through their spec. Examples:
...
Durable consequences remain host-owned. Peer-side canonical ent
spawns are still blocked by `world_ops::SpawnEnt`, and durable damage/tile/
economy changes still go through host requests.

Current code status:

- Peer-owned `PlayerSnapshotsPacket` messages are still the transport for player inputs.
- If a peer snapshot claims ordinary free movement, `Hanging`, `Climbing`, or a fresh upward jump, the host validates the claim through shared ent movement geometry before accepting the body state.
- Accepted hang/climb/jump claims reset host fall timer for that body, preventing stale replay from creating bogus fall damage after a valid local attach or jump.
- Ordinary free body claims use the same shared validator instead of one-off interpolation exceptions.
- Carried, attached, thrown, stunned, dead, and destroyed bodies reject owner-local body claims; host state wins for those external-control cases.
- This is intentionally not durable world authority; tile/ent/economy facts still go through host-owned message lanes.
...
- Do not require exact host frame replay for edge-sensitive movement.
