...
Status: historical host-authoritative plan. The active experiment is now
`docs/plans/input_lockstep_rollback_plan.md` on branch
`net-lockstep-experiment`.

Reason for superseding: the host-authoritative mutation-replication model
made too much gameplay/content code aware of multiplayer authority, prediction,
repair, and per-category replication. That is a poor fit for rapid content work
and future modded content. The new experiment is input lockstep first, then
rollback if det lockstep proves viable.

Keep this file as a record of the previous model and as a source of lessons, but
do not use it as the implementation target while the lockstep/rollback
...
- Do not use pure det lockstep for action gameplay. It protects global
...
- Do not require full-game rollback as the first architecture. It is expensive in
  this engine because `State` currently includes stage tiles, ents, fluids,
...
host-authoritative shared world state.
...
- Local multiplayer does not route through transport. Local player slots operate
  directly on the world through the same `PlayerId -> input -> ent` routing
...
- Clients author/predict their local player motion immediately and send movement
  state. The host may validate/correct against solid world state, stage
  transitions, and death/respawn state.
- The host owns shared world outcomes: stable ent ids, stage
...
- Local actions may present immediately on the acting machine, but durable
  shared results must pass through host ordering before they are treated
  as canonical on other machines.
- Local actions must not create canonical shared-world outcomes on peers. A peer
  may show local-only prediction or cosmetics, but the host creates the
  real ent/tile/inventory result.
- Peers apply durable messages idempotently. If an message arrives late but still
  applies cleanly, apply it. If it conflicts, resolve through host order
...
host authority: clients keep local player controls responsive, but shared
world mutations are requests sent to the host, and the host
...
- The host assigns stable net ids for shared ents.
- Non-player runtime spawns are host-owned by default, even when a client
  requested the spawn. A client-thrown bomb, arrow, rope ball, pot, or loose
  item may be predicted locally for pres, but its canonical shared-world
  physics/damage/tile effects come from the host.
- Peers do not run full gameplay simulation for host-owned non-player
  ents. They may advance pres/anim and may run explicit
  local-interaction seams such as exit prompts, but AI, physics, damage, tile
  breakage, proj contact, and impulses are host-authored.
- The host owns canonical shared world state: stage transitions, tiles,
  loose items, enemies, props, ent health/condition, held/attached state, and
...
- Clients may optimistically present local action intent, but they must not
  ...
  host runs the content path and broadcasts an ordered result or repair.
- Peers must not roll canonical loot, spawn canonical non-player ents, break
  canonical tiles, or apply canonical damage to shared non-player ents.
...
- Cosmetic-only pres uses generic pres commands and remains
  non-authoritative.
...
gameplay should remain direct: ent code mutates ent/tile state immediately
so controls stay simple and responsive.

Current seam: canonical `world_ops` helpers own durable mutation boundaries:

- Host/offline gameplay code calls `world_ops` while applying normal
  content rules.
...
- Ent code must not construct network packets or know about sockets.
- Networking should not know ent-specific rules like sacrifice, shop
...
- In multiplayer peer mode, gameplay code should not perform shared-world
  ...
  the host run the content callback that mutates shared state.
...
`world_ops` is a module, not one fat file: keep ent lifecycle/state, tile
mutation, action requests, pres commands, and future fluid/player/run
...
world_ops::SpawnEnt(state, spawn_spec, source);
world_ops::DeactivateEnt(state, target, reason, source);
world_ops::DamageEnt(state, target, damage_spec, source);
...
- `UseHeldEnt` and `UseBackEnt` carry `use_edge` (`Press` or `Release`)
  plus aim direction.
- Tile/ent/damage requests use their named tile, target, damage, amount,
  velocity, and flag fields.

Pres commands are separate from durable gameplay actions, but they still
use named payload fields. Do not add generic visual slots there either. Current
pres payloads carry explicit fields such as `ent_shake_amount`,
`foreground_shake_amount`, `background_shake_amount`,
`area_ent_shake_amount`, and `shake_radius_tiles`.

These helpers own both sides of a durable mutation:

- mutate the host/offline state through normal content rules;
- enqueue the broad network/result patch when networking is active;
- no-op or apply local-only pres when the result is cosmetic;
- reject or convert peer-side calls into host action requests.
...
- Raw ent/tile/player/run/fluid mutation plus a separate replication emit is
...
- The important boundary is not "everything listens to messages." The important
  boundary is "peers request shared mutations; host applies shared
  mutations."
...
Durable interactions start as source-predicted input/pres only; canonical
shared-world effects are host-owned.

- If a local player or locally-owned held/proj ent hits, carries,
  drops, throws, or otherwise mutates another ent, that local source may
  present intent immediately.
- The source sends an action request to the host. It must not create the
  canonical shared result locally unless this process is the host or
  offline.
- The host orders, confirms, rebroadcasts, or repairs the result.
- This includes remote player targets. Picking up or hitting a remote player may
  be predicted by the holder/attacker, but the host must be able to repair
  held state, damage state, and final velocity.
- Remote-owned projs/melee replicas should not independently author damage
  to our local player. Their owner sends the interaction result to the
  host; peers apply the host-ordered message.
...
- Normal-state movement counters that affect feel, including fall timer
  ...
  lifecycle correction arrives. Periodic host snapshots must not
  downsample these counters and change gameplay thresholds on the owning client.
- Fall damage remains host-authored durable damage. Peers can keep local
  fall counters for prediction/equipment behavior, but they do not apply fall
  damage locally in multiplayer.
- Local movement pres predicts immediately: walk/run, hang/climb/fall
  pose, emote pose, facing, and player anim timing.
- Remote players are replicas. They interpolate or snap from host state;
  they do not run canonical local gameplay.
- Safe local item pres may predict: bat windup, cape open/closed pose,
  jetpack flame, weapon use pose, use sound, recoil pose, and camera shake.
- Attach pres prediction is spec opt-in via
  `predict_attach_use_pres`. The peer still requests authoritative
  held/back use edges; the opt-in only keeps local `UseEnt` state down for
  attachs whose pres/equipment feel is safe to repair.
...
- Canonical tile mutations are not predicted. The host applies break,
  place, and tile-change results.
- Canonical spawned ents are not predicted. Bombs, ropes, arrows, webs,
  thrown pots, loot, and enemies are host-spawned with stable net ids. A
  client may show a cosmetic-only ghost if it never participates in gameplay and
  is deleted when the host result arrives or times out.
- Held, attached, and carried links snap to host state.
- Death, respawn, and stage transitions snap to host state.
- Fluids are host/world-repair state. Local render smoothing is
  pres only.

This is the Terraria-style split we want: clients keep controls responsive, but
world-changing consequences come from host-owned content execution and
broad repair/sync lanes.

## Terraria / Barony Alignment

The target architecture is closer to Terraria and Barony than to det
lockstep:

- Terraria/tModLoader uses stable message categories and server relay rather
  than one packet per item. Examples include proj sync, NPC sync, tile
...
- Barony uses direct semantic packets and helper calls such as movement packets,
  item-use/drop packets, ent snapshots, and server update helpers. It is more
  ad hoc than we want, but the authority shape is similar: clients control their
  own player, and shared ents/world state are server/host updated.

For Splonks, avoid `RequestBreakBox`, `RequestUseTeleporter`, or other
content-specific network families. Use generic categories that carry ent type,
spec id, tool slot, tile coordinate, interaction kind, and compact content
payloads. Host-side content callbacks interpret those ids and produce
generic results.
...
proj sync, item sync, tile manipulation, tile square repair, world data,
...
- Peers should send generic action requests to the host instead of
  authoring durable state locally.
- The host should run normal content callbacks and then publish broad
  result messages.
...
network results without putting packet code inside ents.

Terraria-style protocol shape:

- `PlayerControls` / player state: compact player input/movement/action state.
- `SyncNPC`, `SyncProj`, `SyncItem`: broad full-state sync for moving
  world objects.
- `TileManipulation`, `TileSquare`, section/world data: tile and world repair.
- `ModPacket`: mod-owned extension payloads after mod ident negotiation.
...
server/host owns mutable liquid/world data, clients render and interact
...
- Network packets are broad categories: player state, player action requests,
  ent spawn, ent state patch/snapshot, tile mutation, inventory/player
  patch, stage progression, and pres command.
- Content-specific behavior lives in ent/tool/effect/tile spec code, not
  in `network/`. The host calls the same content callbacks that
  singleplayer uses.
- `network/` may know ids, enums, payload bytes, and replicated field layouts. It
  must not know that a web cannon makes cobwebs, a teleporter phases ents, or
  a machete has a particular anim reset.
- If an spec needs custom durable data beyond the broad fields, add an
  optional spec serializer/deserializer or compact `ContentCommand`
  extension lane. Do not add a one-off packet family named after the item.
- Peers are allowed to predict local player pres and UI feel. Peers are
  not allowed to roll loot, create canonical proj ents, break
  canonical tiles, apply canonical damage, or decide canonical carry/attach
  results.
- Host repair snapshots are expected. The protocol should make wrong
...
- `PlayerMoveState`: local player body pres, anim/control flags,
  health/money/tool/effect summary when needed.
- `PlayerActionRequest`: use tool, use held/back ent, interact, pickup, drop,
  throw, emote, hit tile/ent.
- `EntSpawned`: host-assigned net id, spec id, initial transform,
  velocity/acceleration, owner lane, links, counters, anim.
- `EntStatePatch`: transform, velocity/acceleration, condition, AI state,
  timers/counters, health, collision/proj flags, attach links,
  buyable/shop display state, anim id/frame/speed/loop/finished.
- `EntLifecycle`: deactivate/destroy/splat/collect when the ent should no
...
- `StageProgression`: exit request, accepted transition, stage load, respawn,
  join/leave.
- `PresCommand`: cosmetic sound, shake, particles, phase/warp visuals.
  It must never be the only carrier of durable gameplay state.
- `ActionRequestAck`: host acknowledgment for generic peer requests.
  Peers retry requests until this ack clears the request id; host dedupes
...
- Continuous held/back item use is input-state driven, not item-packet driven.
  ...
  host applies `UseEnt`/`StopUsingEnt` to the attached held/back
  ent every frame from that replicated input. Generic `UseHeldEnt` and
  `UseBackEnt` requests remain edge/ack requests for quick taps and loss
  recovery.
- Peer player snapshots are input packets on the host, not body-authority
  packets. The host simulates remote player control, physics, fall
  damage, stun timers, attachs, and collisions from replicated inputs; it
  must not accept peer snapshot position/velocity as canonical body state.
- Host player snapshots sent to peers are canonical body repair packets
  for every player slot, including the receiving peer's own local player. Peers
  may predict their local player, but they must reconcile to the host
...
Requests are generated by peers or local host input. Requests describe
...
struct NetActionRequest {
    ...
    NetActionKind kind;
    NetEntId source_ent_id;
    NetEntId target_ent_id;
    IVec2 tile_pos;
    IVec2 direction;
    Vec2 world_pos;
    Vec2 velocity;
...
Host apply functions run normal content behavior and emit results:

```cpp
void ApplyActionRequestAsHost(State& state, const NetActionRequest& request);
void ApplyEntSpawnedResult(State& state, const EntSpawnedResult& result);
void ApplyEntStatePatch(State& state, const EntStatePatch& patch);
...
- Host mode may call `Apply*` directly for local host actions.
- Peer mode sends `Request*` and may only do local-only prediction/cosmetics.
- `Apply*` is the only path that performs canonical non-player ent spawns,
  tile changes, loot rolls, inventory changes, and shared damage.
- Result messages are generic and idempotent. They identify content by ent
  type/spec/tool/effect ids, not by hardcoded item packet families.
...
- Ent spawn: tool spawns in `ents/common/throw.cpp`, trap/proj
  spawns, pot/box/chest drops, altar rewards, skeleton skulls, cobra/web shots,
  and tile break drops.
- Ent damage/death: `ents::common::TryDamageEnt` plus death-consumed
  effects such as Meathead/Ankh-style behavior.
- Ent deactivation/destruction: `SetInactive*`, `marked_for_destruction`,
  pickup collection, proj expiry, rope-ball expiry, altar sacrifice, and
  telefrag/crush cleanup.
- Ent impulse/state patch: `ApplyKnockback`, pushblock movement, proj
  attach, carried/held state, and thrown proj ownership.
...
host run the existing content code and emit generic result messages.

Hard rule: peers must not author canonical money, inventory, tool, effect,
ent, tile, or run-state deltas. Peers send input/action requests only. The
host validates them, runs normal gameplay/content code, and broadcasts
the canonical result. This intentionally follows Terraria's broad message-lane
model: world/tile/tile-ent/NPC-style state is server-owned, while clients
request actions or sync only explicitly client-owned control state.

Current generic request coverage:

- `BreakTile`: peer requests a tile break; host runs `BreakStageTiles*`
  and emits ordered tile/spawn results.
- `DamageEnt`: peer requests damage from a locally-owned source to a
  non-local target; host runs `TryDamageEnt` and emits normal damage
  results.
- `HitEnt`: peer requests damage plus hit impulse/proj contact metadata
  from a locally-owned source to a non-local target; host runs
  `TryHitEnt`, applies the content-owned knockback, then emits the final
  damage or state result.
- `CollectEnt`: peer requests pickup/collect resolution; host
  validates overlap and collectability, runs the target's normal contact
  callback, then emits player-state and ent-deactivation results.

Likely next request categories:

- `UseEnt` / `UseTool`: source ent/tool asks host to run the same
  content-owned use callback that offline mode uses.
- `DestroyEnt` or `ContainerOpened`: source requests break/open/sacrifice
  resolution for boxes, pots, chests, and other loot containers.

Canonical result categories:

- `EntSpawnedResult`: host net id, ent type/spec id, position,
  velocity, initial counters/state, holder/attach ids.
- `EntDeactivatedResult`: net id and reason.
- `EntStatePatch`: net id, position, velocity, condition, health,
  held/attached ids, relevant flags, anim/display state when needed.
- `EntInteractionResult`: source id, target id, damage/stun/knockback,
...
- `StageProgressionResult`: exit request accepted, transition start, stage load,
  respawn.
- `PresCommand`: cosmetic-only sound/shake/particles/scripted effect.
...
- In peer mode, canonical tile break/change functions must assert/log and refuse
  unless they are applying a host result.
- In peer mode, canonical non-player ent spawn helpers must assert/log and
  refuse unless they are applying a host result or creating a local-only
  predicted pres ent.
- In peer mode, loot-drop callbacks must not roll canonical drops.
- In peer mode, damage/death/deactivation of host-owned non-player
  ents must be request-only.
- Debug logs should include the authority owner, net id, message id, host
...
- Client arrow trap fires or kills/despawns locally without host result.
- Client pushblock/loose item physics creates durable tile/ent differences.

## Current First Pass Status

Implemented:

- Remote player snapshots with interpolation, including anim state.
- Remote player replicas are display-driven only; they do not run local gameplay
  physics/control.
- Host-ordered tile messages for tile breaks, tile changes, and rope tile
  placement.
- Tool-spawned ent messages for things like grenades/ropes/arrows that need to
  exist on other peers.
- Ent damage/death messages that replicate final health/condition and route
...
- Generic ent state patch messages for moved replicated props such as pushblocks.
- Player bodies use player-derived network ent ids instead of det
  stage ids.
- Ent held/thrown/drop message packets exist for carry ownership, including
  player-carry chains.
- Durable messages now have basic ack/retry boundaries:
  - Peer-authored durable messages stay in a local retry queue until the
    host echoes them.
  - Host-ordered durable messages are resent per remote until that remote
    acks the highest contiguous applied host order.
  - Transient player/ent state patches remain lossy and outside durable
    history.
- The host sends periodic repair state patches for linked non-player
  ents. This repairs shared prop/enemy drift without touching local player
  prediction.
...
- Expand host repair snapshots for shared ents. This is what turns
  transient desync from permanent corruption into a short visual glitch. The
  first pass exists for linked non-player ent state; it still needs a fuller
  stage/ent ownership repair path.
- Convert direct peer-authored durable mutations into action/request/result
  categories owned by the host.
...
Inference: Synthetik's good-feeling co-op is probably not det lockstep.
...
does not appear to use det lockstep.
...
- Ent replication uses broad snapshot packets. `sendEntUDP` sends `ENTU`
  with ent uid, sprite, position, rotation, flags, tick, and velocity.
  Clients receive/create/update ents and assign client-side behavior from
...
- Monster/world state is server-owned enough that monster code frequently calls
  `serverUpdateEntSkill`, `serverUpdateEntFlag`, and related helpers.
  Monster behavior is generally not client-authored. See `src/actmonster.cpp`.

Conclusion: Barony is closest to "server/host authoritative shared world
...
fixes for anim state, held state, damage, item use, and ent ident.

Do not switch to det lockstep or server-only input authority as the
...
- Shared world mutations are host ordered.
- Host owns stable net ids and default ownership for enemies, props,
...
- The host confirms, orders, rebroadcasts, or repairs those outcomes.
- Cosmetic-only pres commands may remain generic and non-authoritative.
...
- `PlayerMoveState`: position, velocity, facing, anim/control state, carried
  attach pres state.
- `PlayerActionRequest`: use tool, use held ent, use back ent, interact,
  pickup, drop, throw, emote, hit tile/ent.
- `EntSpawned`: host-assigned net id, spec id, position, initial
  state, owner lane.
- `EntStateSnapshot`: position, velocity, acceleration, condition, display
  state, anim id/frame, held/attached state, health, timers/counters,
  relevant effect summary.
- `EntStatePatch`: small reliable changes such as condition, sprite/display,
  flags, health, inventory count, effect add/remove.
- `EntInteraction`: damage, stun, knockback, pickup, drop, throw, attach,
...
- `PresCommand`: sound, shake, particle, phase effect, one-shot visual
  command.

For future modded content, prefer a generic extension lane:

- `ContentCommand { spec_id, command_id, payload }`

The core network layer transports and orders it. The registered content
spec interprets it. This keeps the network table stable without hardcoding
...
- Host can validate/correct movement against solid world state and hard
...
- Normal movement correction should be rare and gentle; do not make local
  controls wait for host approval.

### Held / Carried Items

- Predictor: holder's client while held.
- Canonical owner: host for conflict resolution and repair.
- Pickup is a durable message: `PickupEnt(player, item)`.
- The picker applies pickup immediately and sends the action/result to the
  host.
- Conflicting pickups resolve by host message order. Losing peers repair or
...
- Canonical owner: host once the spawn is accepted and assigned a stable
  net id.
- Local actor sees proj/tool results immediately.
- Send action/result categories:
  ...
  - `KillEnt`
  - `SpawnEnt`
- Remote peers may render the proj path approximately. Durable results
  come from host-ordered messages and repair snapshots.

### Enemies / World Props

- Default owner: host.
- A client may predict an active interaction with the ent for local
  responsiveness.
...
- Remote peers accept host-ordered outcomes and may be repaired by later
...
- A peer actor may show immediate local-only break/place pres, but it
  must not permanently modify canonical tile state until the host result
  arrives.
- Host orders tile changes, rebroadcasts if necessary, and repairs
  conflicts.
...
- `message_id`
- `source_player_id`
- `source_ent_vid` or network ent id
...
small window. This matters more than exact det simulation.
...
projs, crates, chests, shops, or sacrifices. The host should run
the normal content callback and emit generic ent, tile, player-state, and
run-state results.
...
generic pres commands for readable one-shots such as sounds, ent
shake, area shake, and named scripted pres effects.

## Network Ident

Local `VID` values are not enough across machines. Use stable player/session ids:

- `NetEntId`: stable for replicated ents during a stage.
- `PlayerId`: stable gameplay player ident. This is not network-owned; local
...
Each peer maps `NetEntId -> local VID`. The host assigns canonical ids
for shared ents. Clients may use provisional prediction ids for immediate
local pres, but those ids must be replaced or confirmed by the
host before the ent is canonical. Local-only effects, particles, and
...
Store networking state outside pure ent behavior. Ents should not know
about sockets or peers. Ent-owned logic should eventually call canonical
`world_ops` helpers for durable mutation. Ent-owned logic should not emit a
local gameplay message and wait for a listener; it should either mutate purely
local pres state or call the appropriate `world_ops` helper/request.
...
Avoid putting ent-specific networking cases in engine code. If `SacAltar`
...
- Unreliable frequent: inputs, player snapshots, ent snapshots, pings.
- Reliable ordered or reliable sequenced: stage load, ent spawn/despawn,
...
- `NetMessage` durable payloads are host-ordered and idempotent once
  received.
- Peer local durable messages retry until the host echoes them back.
- Host-ordered durable messages retry per remote until that remote acks
  the highest contiguous `host_order` it has applied.
- The host prunes ordered durable history only after every connected
  remote has acked it.
- The host periodically sends lossy repair state patches for linked,
  non-player ents. These are intentionally not durable ordered messages; the
  newest repair wins.
- Transient player snapshots and ent state patches intentionally remain
...
- Tool-spawned ents such as grenades, arrows, ropes, bombs.
- Ent damage/death and persistent ent deactivation.
...
2. Smooth remote ents toward received owner snapshots.
...
Basic remote movement comes before replicated tile/ent mutations. Durable
...
- On snapshot receive, store target position, velocity, facing, grounded state,
  and input flags instead of writing ent position directly from the packet.
- Player snapshots must not carry authoritative gameplay semantics such as
  condition, stun/death state, hang/climb state, or anim ids. Those come
  from host-owned state patches/pres, not peer motion packets.
- Each frame, move remote player ents toward their newest target after local
  simulation.
...
3. Peer sends generic `ActionRequest{BreakTile}` to the host.
4. Host applies normal stage-break gameplay if still relevant.
5. Host emits generic `TileBroken` and any spawned loot/ent results.
6. Peers apply host results idempotently. Already-air/already-broken
   tile is a no-op.
7. Debug message log shows requested, received, applied, duplicate, and no-op
   action/result messages.

Then do `EntDamaged` plus deactivation/state results, then
pickup/drop/throw ownership.

## Conflict Resolution

Because clients may predict or report results before host confirmation,
conflicts must be boring and det.
...
- Two players pick up the same item: first ordered `EntHeld` wins. The loser
  drops/clears their local optimistic hold.
- Two players kill the same enemy: first ordered `EntDamaged`/`EntDeactivated`
  result wins. Later damage/deactivation messages against inactive/dead ents
  become no-ops.
...
- One player buys an item while another steals it: message order decides whether
  the buy's generic player/ent state patches or theft/disturbance patches
  apply first.
...
- Hard desync: stage tiles, quest flags, inventory, active ent graph, or shop
  ownership diverge. Host sends a repair snapshot or forces stage resync.

## Local Multiplayer

Local multiplayer should be a simpler version of the same model:

- Multiple controlled player ents in one `State`.
...
1. Define networking ids and message envelope.
   - Add `PlayerId`, `NetEntId`, `NetMessageId`, `StageInstanceId`.
   - Add local mapping between `NetEntId` and `VID`.
   - Add host-order field, even before real networking exists.
...
7. Add reliable message stream transport.
   ...
   - Unreliable stream for frequent player/ent snapshots.
8. Add repair snapshots.
   - Serialize durable stage/ent/tool/quest state.
...
- [x] Added the gameplay player registry spine.
  ...
  - Control intent now resolves through `PlayerId -> ent VID -> inputs`
    before falling back to legacy `controlled_ent_vid`.
  - The playing tick runs control logic for every player slot with an ent;
    `controlled_ent_vid` is fallback for legacy debug possession only.
  - Ent stepping now processes all registered player-slot ents first,
    then skips them in the normal ent pass.
- [x] Added debug local multiplayer bots.
  - `Debug: Network` can add/remove local debug player slots that spawn normal
    player ents and feed random movement/jump inputs through the registry.
...
- [x] Added net message queues and first apply path.
  ...
  - The old manual local-to-ordered drain path was removed. Peers keep pending
    outbound action/pres messages; the host owns ordered result
    messages.
- [x] Add in-process durable message apply point in the gameplay step.
  - The playing tick applies host-ordered messages once per frame.
- [x] Added first real UDP host/join transport.
  ...
  - `Debug: Network` can host a UDP socket, join `host:port`, assign explicit
    host-owned `PlayerId`s for each local player in the joining process,
...
- [x] Added remote player interpolation targets for UDP snapshots.
  - Snapshot receives now update a target per remote `PlayerId`; the remote
    ent moves toward that target after normal local simulation.
...
- [x] Removed peer-authored semantic state from UDP player snapshots.
  - Player snapshots now carry only movement/input hints. Condition, stun/death,
    hang/climb state, and anim state are intentionally excluded so peer
    packets cannot overwrite host-authored gameplay results.
- [x] Added first UDP request/result world mutation: `BreakTile` -> `TileBroken`.
  ...
  - Host-side stage tile breaking emits generic `TileBroken` result
...
- [x] Added first generic shared damage request: `DamageEnt`.
  - Non-authoritative damage attempts from locally-owned sources emit
    `ActionRequest{DamageEnt}` instead of directly applying canonical damage
    to remote/host-owned targets.
  - The host applies the request through normal `TryDamageEnt`, so
  ...
  - Current limitation: damage requests do not yet carry a full authoritative
    ...
    pos/vel once the host applies them, but source-specific hit impulse
    should be folded into the next `HitEnt`/damage request shape rather than
    patched per weapon.
- [x] Added generic shared hit request: `HitEnt`.
  - `HitEnt` carries damage type/amount, knockback velocity, clear flags,
    thrown immunity, and proj-contact metadata.
  - Host-side apply uses `TryHitEnt`, so damage and impulse are
    ordered together and the replicated damage/state result includes the final
    post-hit velocity.
  - Main routed paths now include bat hits, stomp victim hits, body/proj
    contact hits, machete/mattock strikes, pistol hits, arrow ent hits, and
    explosion push hits.
...
- [x] Add reliable-ish host-ordered `TileBroken` as the first durable UDP
  world mutation.
- [x] Add multiple local player ids/ents.
- [x] Add ordered ent carry messages.
  - `EntHeld`, `EntDropped`, and `EntThrown` have packet encoding,
    host relay, and apply paths.
  - Player-carry chains use the normal ent carry references; attach sync
...
- [x] Add ack/retry reliable delivery boundaries for durable messages.
  - Peer local durable messages retry until host echo/ack.
  - Host ordered durable messages retry per peer until ack.
  - Transient player/ent snapshots stay lossy and are never retried.
- [ ] Add loopback host/peer transport and route it through the fuzzer.
...
- Per-ent network owner/host overlay.
- Message log filtered by player/ent/message id.
- Prediction overlay view for local-only tile/ent changes.
...
- Localhost debug-control CLI for querying live processes while reproducing
  ...
  - `scripts/splonksctl --port 41000 ents near 128 64`
  - `scripts/splonksctl --port 41000 ent 0`
...
Add a det multiplayer scenario harness before more feature work. The
goal is to catch desync regressions in CI/local CLI, not to replace manual feel
tests.

Preferred shape:

- Run one host and N peers in-process using a fake transport and no
  renderer/audio.
...
- Each scenario sets a stage, seeds RNG, spawns local/remote players, applies
  ...
  state.
- Compare structured state hashes/diffs for tiles, fluids, ent net ids,
  ent transforms/state/links/health/anim, player inventory/effects,
...
- Push block plus arrow/proj attach.
...
harness compare authoritative durable state, not pres-only particles or
sounds.
...
3. Add a fake transport or process-driven harness that starts one host
...
5. Expand to N peers after the one-host/one-peer path catches tile,
...
- loopback host/peer in one process
...
- predicted-vs-host body deltas, including snap threshold state, timers,
  condition, grounded state, health, and anim frame
...
- Host model: listen-server player host first; dedicated host can
  be a later variant.
...
lanes. The host owns durable shared outcomes; clients keep local player
control responsive and may present local prediction, but canonical tile, ent,
fluid, inventory, run, and stage state comes from host results or repair
