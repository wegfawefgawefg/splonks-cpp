...
Status: historical coordinator-authoritative plan. The active experiment is now
`docs/plans/input_lockstep_rollback_plan.md` on branch
`net-lockstep-experiment`.

Reason for superseding: the coordinator-authoritative mutation-replication model
...
rollback if deterministic lockstep proves viable.
