...
FVec2 GetDirectionAxis(const IVec2& direction) {
    const FVec2 axis = FVec2::New(static_cast<float>(direction.x), static_cast<float>(direction.y));
...
FVec2 GetDirectionOrtho(const FVec2& axis) {
...
void SpawnTeleportSplitEffectAt(
    ...
    const FVec2 axis = GetDirectionAxis(direction);
    const FVec2 ortho = GetDirectionOrtho(axis);
    SpawnEntPhaseParticleAt(ent, graphics, visual_center, FVec2::New(0.0F, 0.0F), (axis * -0.3F) - (ortho * 0.0625F), 1.0F, 0.20F, 0.20F, state);
    SpawnEntPhaseParticleAt(ent, graphics, visual_center, FVec2::New(0.0F, 0.0F), ortho * 0.0375F, 0.25F, 1.0F, 0.25F, state);
    SpawnEntPhaseParticleAt(ent, graphics, visual_center, FVec2::New(0.0F, 0.0F), (axis * 0.3F) - (ortho * 0.0625F), 0.30F, 0.30F, 1.0F, state);
}
...
void SpawnJetpackSmokeAt(State& state, const FVec2& pos) {
    for (int i = 0; i < 16; ++i) {
...
void SpawnExplosionBurstAt(State& state, const FVec2& center, float size) {
...
void SpawnPistolMuzzleSmokeAt(State& state, const FVec2& pos, int direction) {
...
void SpawnPistolImpactAt(State& state, const FVec2& pos, int direction) {
    const int normalized_direction = direction < 0 ? -1 : 1;
    for (int i = 0; i < 3; ++i) {
...
void SpawnTreasurePickupSparklesAt(State& state, const FVec2& center, Color3 color, std::uint32_t count) {
...
void SpawnBaseballBatTrailAt(State& state, const FVec2& from, const FVec2& to) {
...
void PlayScriptedEffect(State& state, Graphics& graphics, const PresCommand& command) {
    if (command.effect_id == ScriptedPresEffectId::JetpackSmoke) {
        SpawnJetpackSmokeAt(state, command.source_pos + FVec2::New(3.0F, 3.0F));
        SpawnJetpackSmokeAt(state, command.source_pos + FVec2::New(-3.0F, 3.0F));
        return;
    }
