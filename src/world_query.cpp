...
sim::AABB ShiftAabb(sim::AABB aabb, sim::FxVec2 delta) {
    aabb.translate(delta);
    return aabb;
}

sim::FxVec2 GetAabbCenter(sim::AABB aabb) {
    return aabb.center();
}
...
std::vector<sim::FxVec2> GetQueryOffsets(const Stage& stage, sim::AABB area) {
