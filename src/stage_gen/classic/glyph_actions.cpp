...
const char* PickRightTikiArmFrameName(DetRng& det_rng) {
...
const char* PickLeftTikiArmFrameName(DetRng& det_rng) {
    switch (det_rng.RandomIntInclusive(0, 2)) {
    case 0:
...
ResolvedRoom ResolveRoom(int room_code, int level_number, bool is_start_room, bool is_end_room, int room_code_above,
    ...
    const RoomTemplateSelection selection = SelectRoomTemplate(
        room_code, is_start_room, is_end_room, room_code_above, jungle_lake_active,
        room_templates, det_rng);
    const std::string glyphs = ExpandObstacles(selection.glyphs, det_rng);
    ...
    for (int y = 0; y < static_cast<int>(room_size.y); ++y) {
        for (int x = 0; x < static_cast<int>(room_size.x); ++x) {
            ...
            const auto roll_classic_ground_block = [&]() {
                return det_rng.RandomIntInclusive(1, 10) == 1
            ...
            if (rule->spawn_chance != EntType::None &&
                det_rng.RandomIntInclusive(1, rule->chance_denominator) == 1) {
                spawn_ent(rule->spawn_chance);
            }
            if (!rule->spawn_random.empty()) {
                spawn_ent(PickWeightedEnt(rule->spawn_random, det_rng));
            }
            ...
            if (!rule->action.empty()) {
                ...
                if (action == "random_brick_or_block") {
                    ...
                    tile = det_rng.RandomIntInclusive(1, 2) == 1 ? roll_classic_ground_block()
                                                              : Tile::Air;
                } else if (action == "jungle_ground") {
                    tile = Tile::Lush;
                } else if (action == "jungle_maybe_ground") {
                    tile = det_rng.RandomIntInclusive(1, 2) == 1 ? Tile::Lush : Tile::Air;
                } else if (action == "jungle_water_or_lush") {
                    tile = det_rng.RandomIntInclusive(1, 2) == 1 ? Tile::WaterSwim : Tile::Lush;
                } else if (action == "jungle_temple_or_lush") {
                    tile = det_rng.RandomIntInclusive(1, 2) == 1 ? Tile::TempleDirt : Tile::Lush;
                } else if (action == "jungle_water_and_lush") {
                    tile = Tile::WaterSwim;
                } else if (action == "jungle_water_and_maybe_lush") {
                    tile = Tile::WaterSwim;
                } else if (action == "ice_dark_or_ice") {
                    tile = det_rng.RandomIntInclusive(1, 10) == 1 ? Tile::IceBlock : Tile::Dark;
                } else if (action == "ice_maybe_dark_or_ice") {
                    if (det_rng.RandomIntInclusive(1, 2) == 1) {
                        tile = det_rng.RandomIntInclusive(1, 10) == 1 ? Tile::IceBlock : Tile::Dark;
                    }
                    ...
                    if (det_rng.RandomIntInclusive(1, 2) == 1) {
                        tile = det_rng.RandomIntInclusive(1, 10) == 1 ? Tile::TempleBlock
                                                                   : Tile::TempleDirt;
                    }
                } else if (action == "temple_lava_or_ground") {
                    tile = det_rng.RandomIntInclusive(1, 2) == 1 ? Tile::Lava : Tile::TempleDirt;
                } else if (action == "maybe_lush") {
                    tile = det_rng.RandomIntInclusive(1, 2) == 1 ? Tile::Lush : Tile::Air;
                } else if (action == "maybe_ice_block") {
                    tile = det_rng.RandomIntInclusive(1, 2) == 1 ? Tile::IceBlock : Tile::Air;
                } else if (action == "maybe_spikes") {
                    tile = det_rng.RandomIntInclusive(1, 3) == 1 ? Tile::Spikes : Tile::Air;
                    ...
                    if (det_rng.RandomIntInclusive(1, rule->chance_denominator) == 1) {
                    ...
                    int trunk_y = y - 1;
                    for (int m = 0; m < 5; ++m) {
                        if (det_rng.RandomIntInclusive(0, m) > 2) {
                            break;
                        }
                        if (trunk_y < 0) {
                            break;
                    ...
                    spawn_shop_item(
                        PickHighEndShopItemType(item_db, existing_stage, room.ent_spawns, det_rng));
                } else if (action == "random_item") {
                    if (is_shop_room) {
                        const std::optional<std::size_t> item_spawn_index =
                            spawn_ent(PickUndergroundItemType(item_db, existing_stage, det_rng));
                        if (is_craps_shop) {
                    ...
                    spawn_shop_item(PickShopItemType(selection.shop_type, existing_stage,
                                                     room.ent_spawns, item_db, shop_db, det_rng));
                    ...
                    if (det_rng.RandomIntInclusive(1, 120) == 1) {
                        spawn_ent(EntType::RubyBig);
                    ...
                    if (det_rng.RandomIntInclusive(1, 10) != 1 && det_rng.RandomIntInclusive(1, 2) == 1) {
                        tile = DirtTileForFamilyTile(existing_stage.border.left.tile);
                    }
                } else if (action == "maybe_snake_or_solid_dirt") {
                    if (det_rng.RandomIntInclusive(1, 10) == 1) {
                        spawn_ent(EntType::Snake);
                    } else if (det_rng.RandomIntInclusive(1, 2) == 1) {
                        tile = DirtTileForFamilyTile(existing_stage.border.left.tile);
                    }
                } else {
