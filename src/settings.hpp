...
struct FluidSettings {
    ...
    FxScalar transfer_per_step = FxScalar::from_int(1);
    FxScalar gravity_x = FxScalar::zero();
    FxScalar gravity_y = FxScalar::from_int(1);
    FxScalar pressure_strength = ToFxScalar(0.35F);
    FxScalar velocity_damping = ToFxScalar(0.88F);
    FxScalar temp_gravity_decay = ToFxScalar(0.92F);
    bool temporal_smoothing_enabled = false;
    FxScalar temporal_smoothing_response = ToFxScalar(0.35F);
    FxScalar render_cutoff_amount = ToFxScalar(0.004F);
    FxScalar water_alpha = ToFxScalar(0.69F);
    bool lighting_enabled = true;
    FxScalar lighting_strength = FxScalar::from_int(1);

    static FluidSettings New();
};

struct WaterEffectSettings {
    FxScalar gravity_scale = ToFxScalar(0.35F);
    FxScalar velocity_damping_x = ToFxScalar(0.82F);
    FxScalar velocity_damping_y = ToFxScalar(0.55F);
    FxScalar move_speed_scale = ToFxScalar(0.5F);
    FxScalar max_fall_speed = ToFxScalar(1.35F);
    FxScalar buoyancy_strength = ToFxScalar(0.55F);
    FxScalar fall_timer_rate = FxScalar::zero();
    FxScalar stomp_damage_scale = FxScalar::zero();
    FxScalar swim_impulse = ToFxScalar(8.70F);
...
struct PlayerTuningState {
    FxScalar gravity_scale = FxScalar::from_int(1);
    FxScalar max_fall_speed = FxScalar::from_int(9);
    FxScalar jump_impulse = ToFxScalar(4.5F);
    FxScalar spring_shoes_jump_impulse_bonus = FxScalar::from_int(1);
    ...
    FxScalar walk_speed = ToFxScalar(2.5F);
    FxScalar run_speed = FxScalar::from_int(4);
    FxScalar move_acc = ToFxScalar(0.5F);
    FxScalar run_acc = ToFxScalar(0.5F);
    FxScalar ground_friction_scale = FxScalar::from_int(1);
    FxScalar air_friction = ToFxScalar(0.85F);

    FxScalar climb_speed = FxScalar::from_int(3);
    FxScalar climb_depart_horizontal_speed = FxScalar::from_int(4);
    FxScalar climb_probe_bias_pixels = FxScalar::from_int(8);
    FxScalar climb_probe_x_scale = ToFxScalar(0.5F);
