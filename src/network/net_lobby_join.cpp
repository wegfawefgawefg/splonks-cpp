...
void HandleJoinRequestAsHost(
    ...
    FxVec2 remote_spawn = GetRemoteSpawnPos(state);
    for (std::size_t i = 0; i < player_ids.size(); ++i) {
        ...
        const FxVec2 spawn_pos = ResolveReconnectSpawnPos(state, retained, i);
        if (i == 0) {
    ...
    const FxVec2 host_spawn = GetPrimaryPlayerSpawnPos(state);
...
void HandleJoinAcceptAsPeer(
    ...
    const FxVec2 host_spawn{accept.host_spawn_x, accept.host_spawn_y};
    const FxVec2 remote_spawn{accept.remote_spawn_x, accept.remote_spawn_y};
    ...
    if (host_slot.ent_vid.has_value()) {
        if (Ent* const host = state.ents.GetEntMut(*host_slot.ent_vid)) {
            host->pos = host_spawn;
            host->vel = FxVec2::zero();
            host->acc = FxVec2::zero();
