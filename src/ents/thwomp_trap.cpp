...
bool ShouldDrop(const Ent& thwomp, const State& state) {
    const FxVec2 thwomp_center = thwomp.GetCenter();
...
void StartDrop(Ent& thwomp) {
    ...
    thwomp.vel = FxVec2::zero();
    thwomp.acc = FxVec2::zero();
}
...
void StartWait(Ent& thwomp, State& state) {
    ...
    thwomp.vel = FxVec2::zero();
    thwomp.acc = FxVec2::zero();
    thwomp.counter_a = FxScalar::from_int(kWaitFrames);
...
void StartReturn(Ent& thwomp) {
    ...
    thwomp.vel = FxVec2{FxScalar::zero(), ToFxScalar(kReturnVelocity)};
    thwomp.acc = FxVec2::zero();
}
...
void StepEntLogicAsThwompTrap(
    ...
    if (IsWaiting(thwomp)) {
        thwomp.vel = FxVec2::zero();
        thwomp.acc = FxVec2::zero();
        thwomp.counter_a -= FxScalar::from_int(1);
        if (thwomp.counter_a <= FxScalar::zero()) {
            StartReturn(thwomp);
        }
    }
}
...
void StepEntPhysicsAsThwompTrap(
    ...
    const FxScalar pre_vel_y = thwomp.vel.y;
    ...
    if (IsDropping(thwomp)) {
        thwomp.vel.y = gfxp::clamp(thwomp.vel.y, FxScalar::zero(),
    ...
    if (IsDropping(thwomp)) {
        const bool hit_bottom = (!was_grounded && thwomp.grounded) ||
                                (pre_vel_y > FxScalar::zero() && thwomp.collided &&
                                 thwomp.vel.y == FxScalar::zero());
        if (hit_bottom) {
