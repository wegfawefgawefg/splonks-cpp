...
void ApplyAFrameGeometryToEnt(std::size_t ent_idx, State& state, const Graphics& graphics) {
    ...
    ent.size = FxVec2::from_int(aframe->pbox.w, aframe->pbox.h);
}
...
FxVec2 GetSpriteTopLeftForEnt(const Ent& ent, const AFrame& aframe) {
    const FxVec2 draw_offset =
        FxVec2::from_int(aframe.draw_offset.x, aframe.draw_offset.y);
    const FxVec2 pbox_offset = FxVec2::from_int(aframe.pbox.x, aframe.pbox.y);
    ...
    return ent.pos - FxVec2::from_int(mirrored_pbox_x, aframe.pbox.y) +
...
FxVec2 GetVisualCenterForEnt(const Ent& ent, const Graphics& graphics, FxVec2 fallback) {
    ...
    const FxVec2 sprite_world_size =
        FxVec2::from_int(aframe->sample_rect.w, aframe->sample_rect.h) *
...
void SetVisualCenterForEnt(Ent& ent, const Graphics& graphics, FxVec2 center) {
    ...
    const FxVec2 draw_offset =
        FxVec2::from_int(aframe->draw_offset.x, aframe->draw_offset.y);
    const FxVec2 pbox_size = FxVec2::from_int(aframe->pbox.w, aframe->pbox.h);
    const FxVec2 pbox_center_offset =
        (pbox_size - FxVec2::from_int(1, 1)) / FxScalar::from_int(2);
...
FxVec2 GetEmitPointForEnt(const Ent& ent, const Graphics& graphics, FxVec2 fallback) {
    ...
    const FxVec2 emit_point = sprite_tl + FxVec2::from_int(
                                                 emit_x,
                                                 aframe->emit_point.y
                                             );
...
FxAABB GetContactAabbForEnt(const Ent& ent, const Graphics& graphics) {
    ...
    const FxVec2 contact_tl =
        sprite_tl + FxVec2::from_int(contact_x, aframe->cbox.y);
    return FxAABB::from_corners(
        contact_tl,
        contact_tl + FxVec2::from_int(aframe->cbox.w - 1, aframe->cbox.h - 1)
    );
}
