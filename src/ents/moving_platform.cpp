...
void SpawnIcyPlatformParticles(const Ent& platform, State& state) {
    ...
    shard.vel = FVec2::New(rng::RandomFloat(-0.35F, 0.35F), rng::RandomFloat(-0.4F, -0.1F));
    shard.svel = FVec2::New(0.0F, 0.0F);
    shard.rotvel = rng::RandomFloat(-0.25F, 0.25F);
    shard.alpha_vel = -0.03F;
    shard.acc = FVec2::New(0.0F, 0.02F);
...
void StepHorizontalPingPong(Ent& platform) {
    const FxScalar min_x = FxScalar::from_int(platform.point_a.x);
    const FxScalar max_x = FxScalar::from_int(platform.point_b.x);
    if (platform.counter_b == FxScalar::zero()) {
        platform.counter_b = FxScalar::from_int(1);
    }

    if (platform.counter_b > FxScalar::zero() && platform.pos.x >= max_x) {
        platform.pos.x = max_x;
        platform.counter_b = -FxScalar::from_int(1);
    } else if (platform.counter_b < FxScalar::zero() && platform.pos.x <= min_x) {
        platform.pos.x = min_x;
        platform.counter_b = FxScalar::from_int(1);
    }

    platform.vel = FxVec2{platform.counter_b * ToFxScalar(kPlatformSpeed),
                             FxScalar::zero()};
}

void StepVerticalPingPong(Ent& platform) {
    const FxScalar min_y = FxScalar::from_int(platform.point_a.y);
    const FxScalar max_y = FxScalar::from_int(platform.point_b.y);
    if (platform.counter_b == FxScalar::zero()) {
        platform.counter_b = FxScalar::from_int(1);
    }

    if (platform.counter_b > FxScalar::zero() && platform.pos.y >= max_y) {
        platform.pos.y = max_y;
        platform.counter_b = -FxScalar::from_int(1);
    } else if (platform.counter_b < FxScalar::zero() && platform.pos.y <= min_y) {
        platform.pos.y = min_y;
        platform.counter_b = FxScalar::from_int(1);
    }

    platform.vel = FxVec2{FxScalar::zero(),
...
void StepCircle(Ent& platform) {
    ...
    const FxScalar circle_path_size =
        FxScalar::from_int(static_cast<int>(kCirclePath.size()));
